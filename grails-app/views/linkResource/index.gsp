			...
			<thead>
				...
				<g:each in="${linkResourceInstanceList}" status="i" var="linkResourceInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${linkResourceInstance.id}">${fieldValue(bean: linkResourceInstance, field: "topic")}</g:link></td>
					
						<td>${fieldValue(bean: linkResourceInstance, field: "createdBy")}</td>
					
						<td>${fieldValue(bean: linkResourceInstance, field: "description")}</td>
